...
namespace SypexDotNet
{
    public class Reader : IReader, IDisposable
    {
        ...
        private const string pyScript = @"from _struct import *
...
def get_location(ip, detailed= False) :
    if _locator is None:
        raise ValueError(""create GeoLocator first"")
    return _locator.get_location(ip, detailed)

def get_locator(file_name, mode) :
    return GeoLocator(file_name, mode)";

        #endregion

        #region Static and Readonly Fields

        private readonly Lazy<ScriptEngine> engine;
        private static Lazy<dynamic> locator;
        ...
        private static SypexResponse MapToModel(string ip, dynamic obj)
        {
            dynamic ctyId = obj["info"]["city"]["id"];
            dynamic cntryId = obj["info"]["country"]["id"];
            dynamic rgionId = obj["info"]["region"]["id"];
            dynamic ctyLat = obj["info"]["city"]["lat"];
            dynamic cntryLat = obj["info"]["country"]["lat"];
            dynamic rgionLat = obj["lat"];
            dynamic ctyLon = obj["info"]["city"]["lon"];
            dynamic cntryLon = obj["info"]["country"]["lon"];
            dynamic rgionLon = obj["lon"];
            dynamic ctyNameEn = obj["info"]["city"]["name_en"];
            dynamic cntryNameEn = obj["info"]["country"]["name_en"];
            dynamic rgionNameEn = obj["info"]["region"]["name_en"];
            dynamic ctyNameRu = obj["info"]["city"]["name_ru"];
            dynamic cntryNameRu = obj["info"]["country"]["name_ru"];
            dynamic rgionNameRu = obj["info"]["region"]["name_ru"];
            dynamic cntryIso = obj["country_iso"];
            dynamic rgionIso = obj["info"]["region"]["iso"];

            int cityId = ctyId is int ? ctyId : 0;
            int countryId = cntryId is int ? cntryId : 0;
            int regionId = rgionId is int ? rgionId : 0;
            decimal cityLat = ctyLat is int ? ctyLat : 0;
            decimal countryLat = cntryLat is int ? cntryLat : 0;
            decimal regionLat = rgionLat is int ? rgionLat : 0;
            decimal cityLon = ctyLon is int ? ctyLon : 0;
            decimal countryLon = cntryLon is int ? cntryLon : 0;
            decimal regionLon = rgionLon is int ? rgionLon : 0;
            string cityNameEn = ctyNameEn is string ? ctyNameEn : string.Empty;
            string countryNameEn = cntryNameEn is string ? cntryNameEn : string.Empty;
            string regionNameEn = rgionNameEn is string ? rgionNameEn : string.Empty;
            string cityNameRu = ctyNameRu is string ? ctyNameRu : string.Empty;
            string countryNameRu = cntryNameRu is string ? cntryNameRu : string.Empty;
            string regionNameRu = rgionNameRu is string ? rgionNameRu : string.Empty;
            string countryIso = cntryIso is string ? cntryIso : string.Empty;
            string regionIso = rgionIso is string ? rgionIso : string.Empty;

            if (string.IsNullOrEmpty(regionNameEn))
            {
                regionNameEn = countryNameEn;
            }
            if (string.IsNullOrEmpty(cityNameEn))
            {
                cityNameEn = regionNameEn;
            }

            if (string.IsNullOrEmpty(regionNameRu))
            {
                regionNameRu = countryNameRu;
            }
            if (string.IsNullOrEmpty(cityNameRu))
            {
                cityNameRu = regionNameRu;
            }
        ...
        public SypexResponse GetLocation(string ip, bool isDetailed)
        {
            dynamic obj = locator.Value.get_location(ip, isDetailed);
            return obj != null ? MapToModel(ip, obj) : SypexResponse.CreateDefaultModel(ip);
        }
